...
export const runningTrainingService = {
  // Obter todos os planos
  getAllPlans: async () => {
    return api.get('/api/running-training');
  },
  
  // Obter um plano específico
  getPlan: async (planId) => {
    return api.get(`/api/running-training/${planId}`);
  },
  
  // Criar um plano pendente de pagamento
  createPendingPlan: async (planData) => {
    return api.post('/api/running-training', planData);
  },
  
  // Solicitar geração assíncrona de um plano que já foi pago
  generateApprovedPlan: async (planId) => {
    return api.post(`/api/running-training/${planId}/generate`);
  },
  
  // Fazer download do plano
  downloadPlan: async (planId) => {
    try {
      const blob = await api.get(`/api/running-training/${planId}/download`);
      ...
      a.href = url;
      a.download = `corrida_${planId}.xlsx`;
  ...
  downloadPlanPdf: async (planId) => {
    try {
      const blob = await api.get(`/api/running-training/${planId}/download-pdf`);
      // Cria um URL temporário para o blob
      const url = window.URL.createObjectURL(blob);
      // Cria um link e simula o clique
      const a = document.createElement('a');
      a.style.display = 'none';
      a.href = url;
      a.download = `corrida_${planId}.pdf`;
      document.body.appendChild(a);
      a.click();
      // Limpa o URL temporário
      window.URL.revokeObjectURL(url);
