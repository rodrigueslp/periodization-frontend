...
export const bikeTrainingService = {
  // Obter todos os planos
  getAllPlans: async () => {
    return api.get('/api/bike-training');
  },
  
  // Obter um plano específico
  getPlan: async (planId) => {
    return api.get(`/api/bike-training/${planId}`);
  },
  
  // Criar um plano pendente de pagamento
  createPendingPlan: async (planData) => {
    return api.post('/api/bike-training', planData);
  },
  
  // Solicitar geração assíncrona de um plano que já foi pago
  generateApprovedPlan: async (planId) => {
    return api.post(`/api/bike-training/${planId}/generate`);
  },
  
  // Fazer download do plano
  downloadPlan: async (planId) => {
    try {
      const blob = await api.get(`/api/bike-training/${planId}/download`);
  ...
  downloadPlanPdf: async (planId) => {
    try {
      const blob = await api.get(`/api/bike-training/${planId}/download-pdf`);
      // Cria um URL temporário para o blob
      const url = window.URL.createObjectURL(blob);
      // Cria um link e simula o clique
      const a = document.createElement('a');
