...
const ViewRunningPlanPage = () => {
    ...
    return (
      <Layout>
        <div className="flex flex-col items-center justify-center py-12">
          ...
          <p className="mt-4 text-gray-600">Carregando plano de corrida...</p>
        </div>
      </Layout>
    );
  }
  ...
  const labels = {
    ...
    diasDisponiveis: 'Dias Disponíveis por Semana',
    volumeSemanalAtual: 'Volume Semanal Atual',
  ...
  return (
    <Layout>
      ...
      <div className="bg-white shadow overflow-hidden sm:rounded-lg mt-6">
        <div className="px-4 py-5 sm:px-6 flex justify-between items-center">
          <div>
            <h3 className="text-lg leading-6 font-medium text-gray-900">
              Plano de Corrida para {plan.athleteName}
            </h3>
        ...
        <div className="border-t border-gray-200">
          <dl>
            ...
            <div className="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              <dt className="text-sm font-medium text-gray-500">{labels.diasDisponiveis}</dt>
              <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{plan.diasDisponiveis} dias por semana</dd>
            </div>
            <div className="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              <dt className="text-sm font-medium text-gray-500">{labels.volumeSemanalAtual}</dt>
              <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{plan.volumeSemanalAtual} km por semana</dd>
            </div>
            <div className="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
            ...
            {/* Tempos e Paces */}
            {(plan.paceAtual5k || plan.paceAtual10k || plan.melhorTempo5k || plan.melhorTempo10k || plan.melhorTempo21k || plan.melhorTempo42k) && (
              <div className="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                <dt className="text-sm font-medium text-gray-500">Tempos e Paces</dt>
                <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                  <ul className="grid grid-cols-2 gap-4">
                    {plan.paceAtual5k && (
                      <li>Pace Atual 5K: {plan.paceAtual5k}</li>
                    )}
                    {plan.paceAtual10k && (
                      <li>Pace Atual 10K: {plan.paceAtual10k}</li>
                    )}
                    {plan.melhorTempo5k && (
                      <li>Melhor Tempo 5K: {plan.melhorTempo5k}</li>
                    )}
                    {plan.melhorTempo10k && (
                      <li>Melhor Tempo 10K: {plan.melhorTempo10k}</li>
                    )}
                    {plan.melhorTempo21k && (
                      <li>Melhor Tempo 21K: {plan.melhorTempo21k}</li>
                    )}
                    {plan.melhorTempo42k && (
                      <li>Melhor Tempo 42K: {plan.melhorTempo42k}</li>
                    )}
                  </ul>
                </dd>
              </div>
            ...
            {plan.dataProva && (
              <div className="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                ...
                <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{formatDateSafely(plan.dataProva)}</dd>
              </div>
            )}
            {plan.preferenciaTreino && (
              <div className="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                <dt className="text-sm font-medium text-gray-500">Período de Treino</dt>
                <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                  {preferenciaTreinoLabels[plan.preferenciaTreino] || plan.preferenciaTreino}
                </dd>
              </div>
            )}
            {plan.localTreino && (
              <div className="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                ...
                <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                  {localTreinoLabels[plan.localTreino] || plan.localTreino}
                </dd>
              </div>
            )}
            {plan.equipamentosDisponiveis && (
              <div className="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                <dt className="text-sm font-medium text-gray-500">Equipamentos Disponíveis</dt>
                <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{plan.equipamentosDisponiveis}</dd>
              </div>
            )}
            {plan.historicoLesoes && (
              <div className="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                <dt className="text-sm font-medium text-gray-500">Histórico de Lesões</dt>
                <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{plan.historicoLesoes}</dd>
              </div>
            )}
            {plan.experienciaAnterior && (
              <div className="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                <dt className="text-sm font-medium text-gray-500">Experiência Anterior</dt>
                <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{plan.experienciaAnterior}</dd>
              </div>
            )}
          </dl>
        </div>
      ...
      {showContent && (
        <div className="mt-8">
          <h3 className="text-lg font-medium text-gray-900 mb-4">Plano de Corrida</h3>
          <div className="bg-white shadow overflow-hidden sm:rounded-lg p-6">
...
export default ViewRunningPlanPage;
